 ...
 * mifosplatform_tenants schema. It sets it to the actual current port of
 ...
 * the mifosplatform_tenants but BEFORE Flyway runs on all tenant schemas.
 *
...
public class TenantDataSourcePortFixService {
	...
	if (!enabled)  {
		logger.info("No schema_server_port UPDATE made to tenant_server_connections table of the mifosplatform_tenants schema, because " + ENABLED + " = false");
		return;
	}
	if (dsp == null) {
		// we don't have any generic mechanism to know the DB port, given just a tenant DataSource
		logger.debug("No schema_server_port UPDATE made to tenant_server_connections table of the mifosplatform_tenants schema (because neither MariaDB4j nor our own Spring Boot DataSourceConfiguration is used in a traditional WAR)");
		return;
	}
		int r = jdbcTemplate
	...
	if ( r == 0 )
		logger.warn("UPDATE tenant_server_connections SET ... did not update ANY rows - something is probably wrong");
			...
			logger.info("Upated "
					+ r
					+ " rows in the tenant_server_connections table of the mifosplatform_tenants schema to the real current host: "
